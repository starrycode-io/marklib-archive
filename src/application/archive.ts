...
export async function generateHTML(id:string, url: string, options?: GenerateHTMLOptions): Promise<void> {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      // exec doesn't support detached option - use spawn for process groups if needed
      childProcess = exec(command, (error: ExecException | null, _stdout: string, _stderr: string) => {
        if (timeoutHandle) {
      ...
      timeoutHandle = setTimeout(() => {
        const process = childProcess as ChildProcess | null;
        if (process && !process.killed) {
          fastify.log.warn(`Timeout reached for ${url}, killing process (PID: ${process.pid})...`);
          ...
          try {
            if (process.pid) {
              ...
              fastify.log.info(`Killed process group: -${process.pid}`);
            }
          } catch (killError) {
            ...
            try {
              ...
              fastify.log.info(`Killed individual process: ${process.pid}`);
            } catch (fallbackError) {
    ...
    const processToKill = childProcess as ChildProcess | null;
    if (processToKill && !processToKill.killed) {
      try {
        fastify.log.warn(`Cleaning up child process (PID: ${processToKill.pid})...`);
        if (processToKill.pid) {
          // Try to kill process group first
          try {
            global.process.kill(-processToKill.pid, 'SIGKILL');
            fastify.log.debug(`Killed process group in cleanup: -${processToKill.pid}`);
          } catch (groupKillError) {
            // Fallback to killing individual process
            try {
              processToKill.kill('SIGKILL');
              fastify.log.debug(`Killed individual process in cleanup: ${processToKill.pid}`);
